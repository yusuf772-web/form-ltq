...
const recapViews = {
    perkembangan: {
        ...
        columns: ["No", "Nama Santri", "Kelas", "Nama Pembimbing", "Total Hafalan", "Hafalan Terakhir", "Jml Halaman Ziyadah", "Jml Halaman Murojaah", "Nilai Itqon", "Nilai Tajwid", "Catatan", "Alfa", "Izin", "Sakit", "Jumlah Hafalan Tasmi", "Hasil Terakhir Tasmi", "Tanggal Terakhir Tasmi"],
        dataKeys: ["namaSantri", "kelas", "namaPembimbing", "totalHafalan", "hafalanTerakhir", "ziyadah", "murojaah", "nilaiItqon", "nilaiTajwid", "catatan", "alfa", "izin", "sakit", "jumlahHafalanTasmi", "hasilTasmi", "tanggalTasmi"]
    },
    absen: {
        title: "Rekap Absen",
...
auth.onAuthStateChanged(async (user) => {
    if (user) {
        ...
        try {
            await (initialAuthToken ? auth.signInWithCustomToken(initialAuthToken) : auth.signInAnonymously());
        } catch (error) {
...
function preloadData() {
    ...
    Object.entries(collectionsToLoad).forEach(([stateKey, collectionName]) => {
        const q = db.collection('artifacts').doc(appId).collection('public').doc('data').collection(collectionName);
        unsubscribes[stateKey] = q.onSnapshot((snapshot) => {
...
function fetchRecapData() {
    ...
    const activeFilterBtn = dom.filterTypeContainer.querySelector('.filter-btn.active');
    if (!activeFilterBtn) return;
    const filterType = activeFilterBtn.dataset.value;
    ...
    const q = submissionsRef.where('timestamp', '>=', startTimestamp).where('timestamp', '<=', endTimestamp);

    if (unsubscribes.submissions) unsubscribes.submissions();
    unsubscribes.submissions = q.onSnapshot((snapshot) => {
...
function compileStandardData(submissions) {
    ...
    for (const student of studentsToProcess) {
        ...
        compiledRecapData.push({
            ...
            catatan: latestForm1.catatanPerkembangan1 || '-',
...
function populateClassFilterButtons() {
    ...
    allBtn.className = 'filter-chip class-filter-btn';
    ...
    allClasses.sort((a, b) => a.name.localeCompare(b.name)).forEach(cls => {
        ...
        btn.className = 'filter-chip class-filter-btn';
...
function exportToExcel() {
    const view = recapViews[activeRecapFilter];
    if (!view || compiledRecapData.length === 0) return;
