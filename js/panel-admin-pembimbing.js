...
function fetchInitialData() {
    ...
    unsubscribes.periods = dataRef.collection('recapPeriods').onSnapshot((snapshot) => {
        allPeriods = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
        renderPeriodsList();
        findAndRenderActivePeriodRekap();
    });

    unsubscribes.holidays = dataRef.collection('settings').doc('holidays').onSnapshot((docSnap) => {
        allHolidays = docSnap.exists ? docSnap.data().dates || [] : [];
        renderHolidaysList();
        if (activePeriod) showRekapForPeriod(activePeriod);
    });
    
    unsubscribes.mentors = dataRef.collection('mentors').onSnapshot((snapshot) => {
...
function populateMonthAndYearFilters() {
    ...
    if (dom.monthFilter) dom.monthFilter.innerHTML = monthOptionsHtml;
    ...
    if (dom.yearFilter) dom.yearFilter.innerHTML = yearOptionsHtml;
    ...
    if (dom.monthFilter) dom.monthFilter.value = savedMonth;
    if (dom.yearFilter) dom.yearFilter.value = savedYear;
}
...
function renderPeriodsList() {
    ...
    if (!container) return;
    container.innerHTML = '';
    const selectedMonthValue = dom.monthFilter?.value;
    const selectedYearValue = dom.yearFilter?.value;
    ...
    filteredPeriods.sort((a, b) => new Date(b.startDate) - new Date(a.startDate)).forEach(period => {
        ...
        item.innerHTML = `
            <div>
                <p class="font-semibold text-white">${period.name || 'Periode Rekap'}</p> 
                <p class="text-xs text-gray-500">${startDate} s/d ${endDate}</p>
            </div>
            <button type="button" class="icon-btn !bg-red-500/10 text-red-500 hover:!bg-red-500/20 delete-period-btn" data-id="${period.id}">
...
function createHolidayListItem(dateString, time = "") {
    if (!dateString || !dom.holidaysListContainer) return;
    ...
    item.innerHTML = `
        <span class="text-sm font-medium text-gray-300">${formattedDate}${displayTime}</span>
        <button type="button" class="icon-btn !w-8 !h-8 !bg-red-500/10 text-red-500 hover:!bg-red-500/20 remove-holiday-btn">
...
async function handleAddPeriod(e) {
    ...
    try {
        await db.collection('artifacts').doc(appId).collection('public').doc('data').collection('recapPeriods').add({ name, startDate, endDate, displayMonthYear });
...
async function handleDeletePeriod(id) {
    showCustomConfirm("Apakah Anda yakin ingin menghapus periode ini?", async () => {
        try {
            await db.collection('artifacts').doc(appId).collection('public').doc('data').collection('recapPeriods').doc(id).delete();
...
async function handleSaveHolidays() {
    ...
    try {
        const holidayDocRef = db.collection('artifacts').doc(appId).collection('public').doc('data').collection('settings').doc('holidays');
        ...
        await holidayDocRef.set({ dates: [...otherHolidays, ...holidayDates] });
...
function findAndRenderActivePeriodRekap() {
    ...
    if (matchingPeriod) {
        ...
        activePeriod = null;
        if (dom.rekapLoader) dom.rekapLoader.innerHTML = `Tidak ada periode rekap yang cocok untuk <b>${getMonthName(selectedMonth)} ${selectedYear}</b>. Silakan buat periode terlebih dahulu.`;
        if (dom.rekapTableWrapper) dom.rekapTableWrapper.innerHTML = '';
...
async function showRekapForPeriod(period) {
    if (!period || allMentors.length === 0) {
        if (dom.rekapLoader) dom.rekapLoader.innerHTML = 'Memuat data pembimbing...';
        if (dom.exportBtn) dom.exportBtn.classList.add('hidden');
        return;
    }
    if (dom.rekapLoader) {
    ...
    if (dom.rekapTableWrapper) dom.rekapTableWrapper.innerHTML = '';
    if (dom.rekapTitle) dom.rekapTitle.textContent = `Rekapitulasi Kehadiran: ${period.name}`;

    try {
        const q = db.collection('artifacts').doc(appId).collection('public').doc('data').collection('dailyAttendance')
            .where('date', '>=', period.startDate)
            .where('date', '<=', period.endDate);
        
        if (unsubscribes.attendance) unsubscribes.attendance();
        
        unsubscribes.attendance = q.onSnapshot((attendanceSnapshot) => {
            ...
            if (dom.rekapLoader) dom.rekapLoader.classList.add('hidden');
        });
...
function renderRekapTable(period, attendanceData, holidays) {
    ...
    if (dom.rekapTableWrapper) dom.rekapTableWrapper.innerHTML = tableHTML;
    lucide.createIcons();
}
...
async function handleAttendanceToggle(e) {
    ...
    cell.innerHTML = '<div class="w-4 h-4 border-2 border-gray-400 border-t-transparent rounded-full animate-spin mx-auto"></div>'; 

    const docId = `${date}_${session}`;
    const docRef = db.collection('artifacts').doc(appId).collection('public').doc('data').collection('dailyAttendance').doc(docId);

    try {
        if (isPresent) {
            await docRef.update({ presentMentors: FieldValue.arrayRemove(mentorName) });
        } else {
            const docSnap = await docRef.get();
            if (docSnap.exists) {
                await docRef.update({ presentMentors: FieldValue.arrayUnion(mentorName) });
            } else {
                await docRef.set({ date, session, presentMentors: [mentorName] });
            }
        }
    } catch (error) {
...
dom.periodsListContainer?.addEventListener('click', (e) => {
    const delBtn = e.target.closest('.delete-period-btn');
    if (delBtn) handleDeletePeriod(delBtn.dataset.id);
});
dom.addHolidayBtn?.addEventListener('click', () => {
    ...
    if (dom.newHolidayDate) dom.newHolidayDate.value = '';
    if (dom.holidayMorning) dom.holidayMorning.checked = false;
    if (dom.holidayAfternoon) dom.holidayAfternoon.checked = false;
});
dom.holidaysListContainer?.addEventListener('click', (e) => {
    const remBtn = e.target.closest('.remove-holiday-btn');
    if (remBtn) remBtn.closest('.holiday-item').remove();
});
